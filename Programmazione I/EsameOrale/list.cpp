...
template <typename T>
T& List<T>::operator[](int prova) const{
    int max=size();


    if(prova>max){
        throw std::overflow_error("Hai oltrepassato la dimensione massima della lista!\n");
    }

    else if(prova<0){
        throw std::underflow_error("Hai definito una posizione impossibile da prendere!\n"); 
    }

    Node<T>* salvo =head->prox;
    
    for(int i=0; i<prova; i++){
        salvo=salvo->prox;
    }

    return salvo->elemento;
