...
//Strutture, che sono protette da un lock, che servono a contarre gli slack time
//Pe runa completezza sono stati aggiunti,  ma potevo non considerarli
//Lo slack esprime la misura tra il tempo di fine e la prossima deadline
//Posso quindi capire se il task è in ritardo rispetto la sua deadline o meno
struct _slack_rt1{
...
//Viene utilizzata per calcolare lo slack time, in modo da controntarlo con la deadline!
...
void* rt_task1(void* parameter){
	...
	while (1) {
		...
		/* Slack Time */
		//Il calcolo dello slack time viene fatto durante l'esecuzione
...
void* rt_task2(void* parameter){
	...
	while (1) {
		...
		/* Slack Time */
		//Anche qui calcolo lo slack time
		clock_gettime(CLOCK_REALTIME, &finish_time);
		if(compare_time(&(th->r),&finish_time)){
...
//Questo è un nuovo thread, non real time, che stampa a video gli slack time, ha comunque un periodo molto più lento
//serve a comprendere mediamente lo slack dei vari task
