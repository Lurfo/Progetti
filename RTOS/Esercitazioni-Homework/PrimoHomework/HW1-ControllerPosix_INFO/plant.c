//------------------- PLANT.C ---------------------- 
//Parte che simula impiento + driver
...
void * plant_loop(void * par) {
	...
	while (keep_on_running)
	{
		...
		//L'impianto non viene modificato in maniera lineare ad ogni ciclo, ma ad ogni ciclo, lancio una moneta e vedo dopo quanto tempo andare a decrementare
		...
		// aggiunge rumore tra -1 e 1, effetto del rumore
		if (count%2 == 0) {
		...
		//La variabile viene aggiornata solo alla fine, evito una sezione critica molto ampia che rallenta gli altri task
		pthread_mutex_lock(&shared_sensor.lock);
...
void * sensor_driver_loop(void* par) {
	...
	int value = 0;
	while (keep_on_running) {
		//Ad ogni ciclo prelevo il valore, lo scrivo a video e lo inserisco nella stringa di messaggio e lo passo poi alla coda
		wait_next_activation(th);
		pthread_mutex_lock(&shared_sensor.lock);
		...
		//Il messaggio inviato non ha nessua priorità
...
//Esegue un comportamento duale rispetto al sensore
//In treoria è un task periodico anche lui, ma non viene creato come tale
//è comunque un task ciclico, perchè il suo comportamento viene scandito rispetto alla receive che ho nel ciclo while
//se gli arrivano regolarmente dei segnali, gira con una velocità dovuta alla ricezione dovuta alla ricezione dei segnali
//(Se non ho nulla da fare infatti mi sospendo), il task è in grado di adattarsi al rate di quello del controllo

//NOTA: Questo consente all'attuatore di mettersi al passo con il controllore quando invia dei messaggi!
void * actuator_driver_loop(void* par) {
	...
	while (keep_on_running) {
		...
		//Faccio una receive bloccante, attendo il messaggio del controllore
...
int main(void)
{
	...
	//L'attuatore non ha bisogno di impostare la struttura di periodic thread, non è periodico, ma ha comunque un attributo perchè sarà lo stesso un task sched FIFO
	//Quasi come se fosse un task aperiodico (nei fatti comunque lo riceve periodicamente grazie al controllore)
	//Ha maggiore priorità, così soddisfa le richieste appena arrivano
	...
	//Questo unlink deve essere fatto qua, poichè il thread attuatore potrebbe bloccarsi nel suo ciclo, sulla receive, quindi la coda non vine mai eliminata
	//e resta sempre nel sistema
