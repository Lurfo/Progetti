...
int main(int argc, const char *argv[])
{
    ...
    {
        perror("wcet: mq_open (req_ds)");
        exit(-1);
    }

    if ((res_ds = mq_open(DSRESPONSE_QUEUE_NAME, O_RDONLY)) == -1)
    {
        perror("wcet: mq_open (res_ds)");
        exit(-1);
    }

    if (mq_send(req_ds, req_message, strlen(req_message) + 1, 1) == -1)
    {
        perror("wcet: send request (req_ds)");
        exit(-1);
    }

    if (mq_receive(res_ds, res_message, MAX_MSG_SIZE * 20, 0) == -1)
    {
        perror("wcet: receive wcet (res_ds)");
        exit(-1);
    }
