...
//IMPOSTO UNA VARIABILE GLOBALE CHE FA ATTIVARE I TASK (se il controller è non è attivo) DA METTERE SUBITO DOPO IL WHILE, IN OGNI TASK PERIODICO
static int start = 0;
...
void initAttr(pthread_attr_t *attr, pthread_mutexattr_t *mAttr, cpu_set_t *cpu){
	...
	/*NOTA RIGUARDO L'AFFINITY: 
	 La funzione di cui sopra, è stata commentata in quanto, andando ad impostare l'affinity dei thread sulla cpu, causa un problema di esecuzione ai task di backup.
	 Si è provato ad aumentare il periodo dei task e a diminuirne la priorità (rispetto ai task del controller), ma non è stato possibile risolvere il problema.
	 Un comportamento singolare è che con l'uso del comando "taskset" (da terminale), l'affinity dei thread viene modificata senza problemi di esecuzione
	 Dunque solo il controller esegue sulla cpu 0 (senza problemi)
	*/

}

//Funzione di inizializzazione dei thread
void initThread(pthread_t *thread, periodic_thread *th, int prio, int period, struct sched_param *myparam, pthread_attr_t *attr, void *startingRoutine){
...
int main(void)
{
	...
	initThread(&acquire_filter_thread, &acquire_filter_th, 50, TICK_TIME, &myparam, &myattr, acquire_filter_loop);

	// CONTROL THREAD
	periodic_thread control_th;
	initThread(&control_thread, &control_th, 45, TICK_TIME*BUF_SIZE, &myparam, &myattr, control_loop);

	// ACTUATOR THREAD
	periodic_thread actuator_th;
	initThread(&actuator_thread, &actuator_th, 45, TICK_TIME*BUF_SIZE, &myparam, &myattr, actuator_loop);

	//CREAZIONE DEL THREAD DI GUARDIA--------------------------------------
		periodic_thread guardia_th; 
		initThread(&guardia, &guardia_th, 40, 2*TICK_TIME*BUF_SIZE, &myparam, &myattr, guard);
		//NOTA: Ha il periodo più grande quindi, secondo RM, ha la priorità minore!
	//--------------------------------------------------------------------

	//ATTIVAZIONE DEL THREAD DS-------------------------------------------
		periodic_thread ds_thread; 
		initThread(&ds, &ds_thread, 60, TICK_TIME/2, &myparam, &myattr, DefSer);
