	...
	while(keep_on_running){
		...
		if(start){//Prendo il tempo attuale e gli sommo il periodo del server
			...
			if(mq_timedreceive(idCodaRequest, mess, sizeof(mess), NULL, &time)==-1){
				...
				sprintf(risp, "ERRORE: Calcolo del WCET non disponibile nel backup, ripristinare il controllore principale\n"); 
...
int main(void)
{
	...
	//Imposto il protocollo per i semafori
	...
	//Inizializzazione del semaforo per il buffer
