...
unsigned long filterWCET;
unsigned long controlWCET;
unsigned long actuatorWCET;
...
void * acquire_filter_loop(void * par) {
	...
	while (keep_on_running)
	{
		...
		tempWCET=difference_ns(&fine, &inizio);
...
void *DefSer(void *par){
	...
	while(keep_on_running){
		...
		if(mq_timedreceive(idCodaRequest, mess, sizeof(mess), NULL, &time)==-1){
			...
			sprintf(risp, "Avg:%d\nControl:%d\nBuffer:%d, %d, %d, %d, %d\nReference:%d\nFilterWCET: %ldus\nActuatorWCET: %ldus\nControlWCET: %ldus\n", tempAvg, tempControl, tempBuff[0], tempBuff[1], tempBuff[2], tempBuff[3], tempBuff[4], tempRef, filterWCET, actuatorWCET, controlWCET);
