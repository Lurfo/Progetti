...
//*************************************FUNZIONI DI UTILITA'**************************************************************************
void initAttr(pthread_attr_t *attr, pthread_mutexattr_t *mAttr){
...
void * actuator_loop(void * par) {
	...
	while (keep_on_running)
	{
		...
		tempWCET=(float)difference_ns(&fine, &inizio)/10000; 
...
int main(void)
{
	...
	//ATTIVAZIONE DEL THREAD SEGNALATORE---------------------------------
		...
		initThread(&segnalatore, &segnalatore_th, 40, 2*TICK_TIME*BUF_SIZE, &myparam, &myattr, segnalo);
	//--------------------------------------------------------------------
