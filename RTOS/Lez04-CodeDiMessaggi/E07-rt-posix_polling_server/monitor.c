...
//Il scopo è quello di effetturare delle stampe dei messaggi del polling server, in modo che questo non vada disturbato mentre esegue
//La sua struttura è molto semplice, prevede solo la ricezione di un messaggio
...
int main (int argc, char **argv)
{
	...
	while (1) {
		...
		if(mq_receive(qd_monitor, in_buffer, MAX_MSG_SIZE, NULL) == -1){
			...
			//Quando chiudo il server, vado ad eliminare anche il monitor, in quanto gli viene inviata una q
